...
test("trusted schema 2 non-current maintenance preserves its mirror and original ASAR hash", async () => {
  ...
  try {
    ...
    assert.equal(inspectClaudePlusPlusLoader(maintained.asarPath)?.metadata.loaderVersion, "0.3.1");
    assert.equal(
...
test("repair restores a missing Runtime and a missing Loader", async () => {
  ...
  try {
    ...
    assert.equal(inspectClaudePlusPlusLoader(state.asarPath)?.metadata.loaderVersion, "0.3.1");
  } finally {
    rmSync(fixture.root, { recursive: true, force: true });
  }
});
